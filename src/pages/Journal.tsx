...
export function Journal() {
  ...
  const [entries, setEntries] = useState<JournalEntry[]>([
    { id: "1", symbol: "EURUSD", pnl: 1240.00, date: "24 OCT 2023", type: "LONG", strategy: "SCALP", notes: "", emotions: [], tags: ["Breakout"], proof: null, rating: 5, checklist: [{label: "Checked higher timeframe", checked: false}, {label: "Risk within limits", checked: false}, {label: "Fits my trading plan", checked: false}, {label: "Key levels identified", checked: false}, {label: "Economic calendar checked", checked: false}], tradeType: "Scalp", entryPrice: "1.0542", exitPrice: "1.0589", duration: "2h 14m" },
    { id: "2", symbol: "XAUUSD", pnl: -450.20, date: "23 OCT 2023", type: "SHORT", strategy: "TREND", notes: "", emotions: [], tags: ["Trend"], proof: null, rating: 3, checklist: [{label: "Checked higher timeframe", checked: false}, {label: "Risk within limits", checked: false}, {label: "Fits my trading plan", checked: false}, {label: "Key levels identified", checked: false}, {label: "Economic calendar checked", checked: false}], tradeType: "Trend Following", entryPrice: "1950.00", exitPrice: "1945.00", duration: "4h 30m" },
  ]);
                ...
                <div className="p-5">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      ...
                      <p className="text-[10px] text-on-surface-variant font-label uppercase tracking-widest mt-0.5">{entry.date}</p>
                    </div>
                    <div className={cn(
                  ...
                  <div className="flex items-center gap-3">
                    <span className={cn(
                      ...
                      entry.type === 'LONG' ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400"
                    )}>
                      {entry.type}
                    </span>
        ...
        <div className="flex flex-col h-[calc(100vh-180px)] glass-card rounded-2xl border border-white/5 overflow-hidden bg-black/20 backdrop-blur-xl">
          ...
          <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
            <div className="flex items-center gap-6">
              ...
              <div>
                <div className="flex items-center gap-3 mb-1">
                  <h2 className="font-headline text-3xl text-white tracking-tighter">{selectedEntry.symbol}</h2>
                  <span className={cn(
                    "px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest",
                    selectedEntry.pnl >= 0 ? "bg-primary/20 text-primary border border-primary/30" : "bg-rose-500/20 text-rose-400 border border-rose-500/30"
                  )}>
                    {selectedEntry.pnl >= 0 ? "Winner" : "Loser"}
                  </span>
                </div>
                <div className="flex items-center gap-4 text-on-surface-variant">
                  <span className="text-[10px] font-label uppercase tracking-widest flex items-center gap-1.5">
                    {selectedEntry.pnl >= 0 ? <TrendingUp className="w-3 h-3 text-primary" /> : <TrendingDown className="w-3 h-3 text-rose-400" />}
                    {selectedEntry.pnl >= 0 ? "Long" : "Short"} Position
                  </span>
            ...
            <div className="text-right">
              ...
              <h3 className={cn(
                ...
                selectedEntry.pnl >= 0 ? "text-primary" : "text-rose-400"
              )}>
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-10 no-scrollbar">
            ...
            <div className="grid grid-cols-4 gap-6">
              {[
                { label: "Symbol", value: selectedEntry.symbol, icon: Target },
                { label: "Entry", value: selectedEntry.entryPrice, field: "entryPrice", icon: ArrowUpRight },
                { label: "Exit", value: selectedEntry.exitPrice, field: "exitPrice", icon: ArrowDownRight },
                { label: "Duration", value: selectedEntry.duration, field: "duration", icon: Clock }
              ].map((item, idx) => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs font-bold text-white">{selectedEntry.rating}/10</span>
                </div>
                <div className="pt-2">
                  <input 
                    type="range" min="1" max="10" value={selectedEntry.rating}
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-4">
                {['😰', '😥', '😎', '😐', '🤯'].map(emo => (
                  <button 
                    key={emo} 
                    onClick={() => updateEntry({ emotions: selectedEntry.emotions.includes(emo) ? selectedEntry.emotions.filter(e => e !== emo) : [...selectedEntry.emotions, emo] })}
                    className={cn(
                      "w-14 h-14 rounded-2xl flex items-center justify-center text-2xl transition-all duration-300 border",
                      selectedEntry.emotions.includes(emo) 
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xs font-bold text-emerald-400">{selectedEntry.checklist.filter(c => c.checked).length}/5 Complete</span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                {selectedEntry.checklist.map((item, idx) => (
                  <button 
                    key={idx} 
                    onClick={() => updateEntry({ checklist: selectedEntry.checklist.map((c, i) => i === idx ? {...c, checked: !c.checked} : c) })}
                    className={cn(
              ...
              >
                {selectedEntry.proof ? (
                  <div className="relative w-full h-full">
                    <img src={selectedEntry.proof} alt="Proof" className="w-full h-full object-cover" />
