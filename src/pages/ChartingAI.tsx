...
export function ChartingAI() {
  ...
  // Use a callback ref pattern + setTimeout to ensure the container 
  // has real pixel dimensions in the DOM before we call createChart
  const initChart = useCallback((node: HTMLDivElement | null) => {
    if (!node) return;
    // Store ref for resize
    ...
    // Wait for browser to paint and give the div real dimensions
    requestAnimationFrame(() => {
      const rect = node.getBoundingClientRect();
      console.log('[ChartingAI] Container dimensions:', rect.width, rect.height);

      // Fallback: if still 0, use window dimensions
      const w = rect.width > 0 ? rect.width : window.innerWidth - 340;
      const h = rect.height > 0 ? rect.height : window.innerHeight - 120;
  ...
  return (
    <div className="flex flex-col min-h-screen pb-10">
      ...
      <div className="px-4 md:px-8 flex-1 flex gap-4 overflow-hidden" style={{ height: 'calc(100vh - 120px)' }}>

        {/* ── Trade List ── */}
        <div className="w-64 shrink-0 flex flex-col overflow-hidden border border-white/5 rounded-2xl bg-[#0D0D18]">
          ...
          {accountTrades.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-gray-600 p-6 text-center gap-3">
              <Target className="w-8 h-8 opacity-20" />
              <p className="text-[10px] font-bold uppercase tracking-widest opacity-40">No trades</p>
            </div>
          ) : (
              ...
              {accountTrades.map(trade => (
                ...
                >
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <span className={cn('text-xs font-bold tnum', trade.pnl >= 0 ? 'text-[#1ED760]' : 'text-[#E5534B]')}>
                      {trade.pnl >= 0 ? '+' : ''}{trade.pnl.toFixed(2)}
                    </span>
                  </div>
                  <div className="flex items-center gap-1.5 text-[9px] uppercase font-bold tracking-widest text-gray-500">
                    <span className={trade.action === 'BUY' ? 'text-[#1ED760]' : 'text-[#E5534B]'}>
                      {trade.action === 'BUY' ? 'LONG' : 'SHORT'}
                    </span>
                    <span>•</span>
                    <span>{trade.strategy || 'SCALP'}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="flex-1 relative rounded-2xl border border-white/5 overflow-hidden bg-[#0A0A12]">
          ...
          {/* 
            ...
            Height/width is 100% of the parent which has an explicit calc() height.
          */}
          <div
            ref={initChart}
          ...
          {selectedTrade && (
            ...
            >
              ...
              <button onClick={stepForward} disabled={isPlaying || currentIndex >= fullData.length - 1}
